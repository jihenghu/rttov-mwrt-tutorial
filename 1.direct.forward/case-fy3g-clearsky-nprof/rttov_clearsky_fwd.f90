...
subroutine rttov_clearsky_fwd(npix,lon,lat,plevel,qw,ta,skt,psrf,t2m,&
...
!! ===========================================================================
  INTEGER  :: npix
  INTEGER,dimension(npix)  :: ls,dem
  real*4,dimension(npix)   :: lon,lat
  real*4,dimension(npix)   :: sea,sez,soa,soz
  real*4,dimension(npix)   :: skt,psrf,t2m,u10,v10,snowc,smc
  real*4, dimension(29) :: plevel
  real*4, dimension(29,npix) :: qw,ta
  real*4, dimension(10,npix) :: TBsimu,Emiss
!! ===========================================================================
  ...
  DO iprof = 1, nprof
    ...
    profiles(iprof) % t(:)= ta(:,iprof)
    profiles(iprof) % q(:)= qw(:,iprof)
	! print*,profiles(iprof) % q(:)

    ! 2 meter air variables
    profiles(iprof) % s2m % t= t2m(iprof)
    profiles(iprof) % s2m % q= qw(nlevels,iprof)
    profiles(iprof) % s2m % p= psrf(iprof)
    profiles(iprof) % s2m % u= u10(iprof) 
    profiles(iprof) % s2m % v= v10(iprof)
    profiles(iprof) % s2m % wfetc=0


    ! Skin variables
    profiles(iprof) % skin % t= skt(iprof)       
    profiles(iprof) % skin % salinity= 35.0
    profiles(iprof) % skin % snow_fraction=snowc(iprof)
    profiles(iprof) % skin % soil_moisture=smc(iprof)
    profiles(iprof) % skin % fastem= (/3.0, 5.0, 15.0, 0.1, 0.3/)  
   
    ! Surface type and water type
    profiles(iprof) % skin % surftype= ls(iprof)
    profiles(iprof) % skin % watertype= 1


    ! Elevation, latitude and longitude
    profiles(iprof) % elevation= dem(iprof)/1000.
    profiles(iprof) % latitude= lat(iprof)
    profiles(iprof) % longitude= lon(iprof)

    ! Satellite and solar angles
    profiles(iprof) % zenangle= sez(iprof) !53.1
    profiles(iprof) % azangle = sea(iprof)
    profiles(iprof) % sunzenangle= soz(iprof)
    profiles(iprof) % sunazangle = soa(iprof)
  ...
  DO iprof = 1, nprof
    ...
    TBsimu(:,iprof)=radiance%bt((1+joff):nchannels+joff)
    Emiss(:,iprof)=emissivity((1+joff):nchannels+joff) % emis_out
  ENDDO
