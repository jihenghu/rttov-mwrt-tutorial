...
!! Or, specify YYYYMMDD to looping process all files in a specific day
	...
	pwd="/data04/1/hjh/rttov.mlse.algor/2.cloud.forward/case.simu.scatt/"
	era5p="/data04/1/hjh/rttov.mlse.algor/2.cloud.forward/case.simu.scatt/ERA5/"
...
702	read(701,*,end=200) FY_Name 
	...
	fileout="output/RTTOV_TELSEM_CLOUDY_SIMU_Swath_"//trim(adjustl(FY_Name))
...
!! collocate RTTOV inputs
	...
	! do iscan=1,nscan
		! print*,iscan, nscan
		! Call Rttov 		
		! call rttov_cloudy_fwd(npix,FYlon(:,iscan),FYlat(:,iscan),plevel,phalf,&
				! qw_3d(:,:,iscan),ta_3d(:,:,iscan),&
				! cc_3d(:,:,iscan),iwc_3d(:,:,iscan),lwc_3d(:,:,iscan),&
				! rwc_3d(:,:,iscan),swc_3d(:,:,iscan),&
	...
	! Call Rttov 		
	call rttov_cloudy_fwd_telsem(imonth,nscan,npix,FYlon,FYlat,plevel,phalf,&
